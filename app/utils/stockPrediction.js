 ...
 */
export const calculateStockOutPrediction = (articleId, sales, currentStock, precalculatedSoldQty) => {
  if (currentStock <= 0) {
    return { daysRemaining: 0, status: 'critical', dailyVelocity: 0 };
  }

  let totalSold = 0;
  if (precalculatedSoldQty !== undefined && precalculatedSoldQty !== null) {
    totalSold = Number(precalculatedSoldQty);
  } else {
