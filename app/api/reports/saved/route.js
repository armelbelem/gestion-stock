...
export async function GET(request) {
  ...
  try {
    ...
    const [rows] = await db.query('SELECT * FROM saved_reports WHERE report_type = ?', [type]);
    if (rows.length === 0) {
...
export async function POST(request) {
  ...
  try {
    ...
    const { type, fileAName, fileBName, data } = body;
    ...
    await db.query(
      `INSERT INTO saved_reports (report_type, file_a_name, file_b_name, report_data, generated_by, updated_at)
       VALUES (?, ?, ?, ?, ?, NOW())
       ON DUPLICATE KEY UPDATE
         ...
         updated_at = NOW()`,
      [type, fileAName, fileBName, reportDataStr, generatedBy]
    );
...
export async function DELETE(request) {
  ...
  try {
    await ensureTableExists();
    await db.query('DELETE FROM saved_reports WHERE report_type = ?', [type]);
    return NextResponse.json({ success: true });
  } catch (err) {
